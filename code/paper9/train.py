...
def main():
    ...
    # Lists to store training curve values for plotting
    epochs_list = []
    clean_acc_list = []
    bd_acc_list = []
    cross_acc_list = []
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        clean_acc_list.append(best_clean_acc.item())
        bd_acc_list.append(best_bd_acc.item())
        cross_acc_list.append(best_cross_acc.item())
    # Create and save the training curve plots
    plt.plot(epochs_list, clean_acc_list, label='Clean Accuracy')
    plt.plot(epochs_list, bd_acc_list, label='Backdoor Accuracy')
    if opt.cross_ratio:
        plt.plot(epochs_list, cross_acc_list, label='Cross Accuracy')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
